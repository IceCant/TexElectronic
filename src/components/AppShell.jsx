...
export function ScannerModal({ onClose }) {
  const { state } = useData(); const navigate=useNavigate(); const videoRef=useRef(null); const lastScanRef=useRef("");
  const [barcode,setBarcode]=useState(""); const [matchError,setMatchError]=useState(""); const [cameraError,setCameraError]=useState(""); const [cameraStatus,setCameraStatus]=useState("Starting rear camera…");
  const openExactProduct=useCallback(value=>{const normalizedBarcode=value.trim();if(!normalizedBarcode)return false;const product=state.products.find(item=>item.barcode===normalizedBarcode);if(!product){setBarcode(normalizedBarcode);setMatchError(`No product matches barcode ${normalizedBarcode}. Try another code or enter it manually.`);return false;}onClose();navigate(`/products/${product.id}`);return true;},[navigate,onClose,state.products]);
  useEffect(()=>{let disposed=false;let controls;const start=async()=>{try{if(!navigator.mediaDevices?.getUserMedia)throw new Error("Camera scanning is not supported by this browser.");const { BrowserMultiFormatReader }=await import("@zxing/browser");if(disposed)return;const reader=new BrowserMultiFormatReader();controls=await reader.decodeFromConstraints({video:{facingMode:{ideal:"environment"}}},videoRef.current,(result)=>{if(!result||disposed)return;const value=result.getText().trim();if(!value||value===lastScanRef.current)return;lastScanRef.current=value;setMatchError("");if(openExactProduct(value))controls?.stop();});if(!disposed)setCameraStatus("Camera ready — place one barcode inside the frame");}catch(error){if(disposed)return;setCameraStatus("");setCameraError(error?.name==="NotAllowedError"?"Camera permission was denied. Allow camera access in browser settings or enter the barcode below.":"Camera could not start. Enter the barcode below instead.");}};start();return()=>{disposed=true;controls?.stop();};},[openExactProduct]);
  const submit=event=>{event.preventDefault();setMatchError("");openExactProduct(barcode);};
  return <Modal title="Scan product barcode" onClose={onClose}><form className="stack-form scanner-form" onSubmit={submit}><div className="scanner-camera"><video ref={videoRef} muted playsInline aria-label="Live barcode camera"/><i aria-hidden="true"/></div>{cameraStatus&&<div className="scanner-status" aria-live="polite"><span/>{cameraStatus}</div>}{cameraError&&<div className="form-error" role="alert">{cameraError}</div>}<label>Enter barcode manually<input inputMode="numeric" value={barcode} onChange={event=>setBarcode(event.target.value)} placeholder="885000210041"/></label>{matchError&&<div className="form-error" role="alert">{matchError}</div>}<div className="modal-actions"><button type="button" onClick={onClose}>Cancel</button><button className="primary" type="submit"><Barcode/>Find product</button></div></form></Modal>;
}
